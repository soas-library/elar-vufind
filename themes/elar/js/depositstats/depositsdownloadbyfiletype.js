function filterELARWebTrafficDownloadbyfiletypeDeposit(deposit) {
    ...
    checkDownloadFilesByFileType(yearFrom, monthFrom, dayFrom, yearTo, monthTo, dayTo,funding_body, current_superseded, deposit);
 
    document.getElementById("statisticsFormFilesByFileType").action="Home?yearFrom="+yearFrom+"&monthFrom="+monthFrom+"&dayFrom="+dayFrom+"&yearTo="+yearTo+"&monthTo="+monthTo+"&dayTo="+dayTo+"&funding_body="+funding_body+"&current_superseded="+current_superseded+"&deposit="+deposit+"&source=ajax";
...
function checkDownloadFilesByFileType (yearFrom, monthFrom, dayFrom, yearTo, monthTo, dayTo, funding_body, current_superseded,deposit) {

  var content = '';

  $.ajax({
    dataType: 'json',
    url: path + '/AJAX/JSON?method=getDownloadFilesByFileTypeDeposit',
    data: {yearFrom:yearFrom, monthFrom:monthFrom, dayFrom:dayFrom, yearTo:yearTo, monthTo:monthTo, dayTo:dayTo, funding_body:funding_body, current_superseded:current_superseded, deposit:deposit},
