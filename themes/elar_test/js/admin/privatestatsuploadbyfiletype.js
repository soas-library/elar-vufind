function filterELARWebTrafficUploadbyfiletypePrivate() {
    ...
    checkUploadFilesByFileType(yearFrom, monthFrom, dayFrom, yearTo, monthTo, dayTo,funding_body, current_superseded);
 
    document.getElementById("statisticsFormFilesByFileType").action="PrivateStatsUploadbyfiletype?yearFrom="+yearFrom+"&monthFrom="+monthFrom+"&dayFrom="+dayFrom+"&yearTo="+yearTo+"&monthTo="+monthTo+"&dayTo="+dayTo+"&funding_body="+funding_body+"&current_superseded="+current_superseded+"&source=ajax";
...
function checkUploadFilesByFileType (yearFrom, monthFrom, dayFrom, yearTo, monthTo, dayTo, funding_body, current_superseded) {

  var content = '';

  $.ajax({
    dataType: 'json',
    url: path + '/AJAX/JSON?method=getUploadFilesByFileType',
