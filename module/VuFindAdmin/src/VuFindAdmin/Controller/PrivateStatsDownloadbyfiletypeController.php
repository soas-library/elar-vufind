 ...
 * @link     http://vufind.org   Main Site
 */
class PrivateStatsDownloadbyfiletypeController extends AbstractAdmin
{
    /**
	...
	if (empty($dayTo)&& $dayTo<>"0") $dayTo = $current_day;
        ...
        if (empty($currentSuperseded)|| empty($currentSuperseded[0])) {$currentSuperseded="Both"; }
	...
	$downloadByFileType = $stats->getByFileType($yearFrom, $monthFrom, $dayFrom, $yearTo, $monthTo, $dayTo,$fundingBody, $currentSuperseded);
	...
	$years = array_unique (array_merge (array_filter($yearsStats)));
	arsort($years);
	//print_r($years);
	...
	$results [] = $years;
        ...
        if (isset($_POST['btnExportarExcelDownloadedFilesByFileType'])){
                $this->exportarexcelDownloadedFilesByFileType($downloadByFileType,$yearFrom,$monthFrom,$dayFrom,$yearTo,$monthTo,$dayTo,$fundingBody,$currentSuperseded,$source);
        }


        $view = $this->createViewModel(array('results' => $results, 'fundingbodies'=> $fundingBodies, downloadByFileType=>$downloadByFileType));
        $view->setTemplate('admin/privatestatsdownloadbyfiletype/home');
    ...
    public function exportarexcelDownloadedFilesByFileType($results,$yearFrom,$monthFrom,$dayFrom,$yearTo,$monthTo,$dayTo,$fundingBody, $currentSuperseded,$source){

        if ($source=="ajax"){
    	    $stats = $this->getTable('DownloadStat');
            $results = $stats->getByFileType($yearFrom, $monthFrom, $dayFrom, $yearTo, $monthTo, $dayTo,$fundingBody, $currentSuperseded);
        }
