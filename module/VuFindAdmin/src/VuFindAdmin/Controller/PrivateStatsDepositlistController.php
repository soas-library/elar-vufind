 ...
 * @link     http://vufind.org   Main Site
 */
class PrivateStatsDepositlistController extends AbstractAdmin
{
    /**
	...
	//$month="0";
    	...
    	$latResourcesArrayFirst = $tableLatResources->getAllResourcesByDeposit($yearFrom, $monthFrom, $dayFrom, $yearTo, $monthTo, $dayTo, $fundingBody, $currentSuperseded);
	...
	if (isset($_POST['btnExportarExcelDepositos'])){
		$this->exportarexcelDepositos($latResourcesArrayFirst,$yearFrom,$monthFrom,$dayFrom,$yearTo,$monthTo,$dayTo,$fundingBody,$currentSuperseded,$source);
	}

        $view = $this->createViewModel(array('results' => $results, 'fundingbodies'=> $fundingBodies, 'resources'=>$latResourcesArrayFirst,userdownload=>$userDownload,uploadedfiles=>$uploadedFiles));
        $view->setTemplate('admin/privatestatsdepositlist/home');
    ...
    public function exportarexcelDepositos($results,$yearFrom,$monthFrom,$dayFrom,$yearTo,$monthTo,$dayTo,$fundingBody,$currentSuperseded,$source){
    	

        if ($source=="ajax"){
        	$tableLatResources= $this->getTable('LatResources');
        	$results = $tableLatResources->getAllResourcesByDeposit($yearFrom,$monthFrom,$dayFrom,$yearTo,$monthTo,$dayTo,$fundingBody, $currentSuperseded);
