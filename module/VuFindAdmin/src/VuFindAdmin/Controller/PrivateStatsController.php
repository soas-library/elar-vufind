...
class PrivateStatsController extends AbstractAdmin
{
	...
	$uploadedFiles = $tableLatResources->getDepositUploadedFilesStatistics($yearFrom, $monthFrom, $dayFrom, $yearTo, $monthTo, $dayTo);
        $countUploadedFiles = $tableLatResources->getCountDepositUploadedFilesStatistics($yearFrom, $monthFrom, $dayFrom, $yearTo, $monthTo, $dayTo);

        //$uploadedFiles = $tableLatResources->getDepositUploadedFilesPrivateStatistics('2017', '01', '11', '2017', '02', '11');
	//echo "<pre>";
	...
	if (isset($_POST['btnExportarExcelUserDownload'])){
        ...
        if (isset($_POST['btnExportarExcelUploadedFiles'])){
                $this->exportarexcelUploadedFiles($uploadedFiles,$yearFrom,$monthFrom,$dayFrom,$yearTo,$monthTo,$dayTo,$source);
        }


        $view = $this->createViewModel(array('results' => $results, 'resources'=>$latResourcesArrayFirst,userdownload=>$userDownload,countUserDownload=>$countUserDownload, uploadedfiles=>$uploadedFiles, countUploadedFiles=>$countUploadedFiles));
	...
	$objSheet ->getCell('A'.$contador)->setValue('"'.$this->translate('Month'). '","' .$this->translate('Year'). '","' .$this->translate('page hits'). '","' .$this->translate('user logins'). '","' .$this->translate('clics_per_visit').'","' .$this->translate('duration_of_visit').'","' .$this->translate('files with O access').'","' .$this->translate('files with U access').'","' .$this->translate('files with S access').'"');
    ...
    public function exportarexcelUploadedFiles($results,$yearFrom,$monthFrom,$dayFrom,$yearTo,$monthTo,$dayTo,$source){

        if ($source=="ajax"){
	    $tableLatResources= $this->getTable('LatResources');
            $results = $tableLatResources->getDepositUploadedFilesPrivateStatisticsCsv($yearFrom, $monthFrom, $dayFrom, $yearTo, $monthTo, $dayTo);
	...
	$objSheet ->getCell('A'.$contador)->setValue('"'.$this->translate('Deposit ID'). '","' .$this->translate('Deposit Title'). '","' .$this->translate('Uploaded date'). '","' .$this->translate('bundle ID'). '","' .$this->translate('File Name'). '","' .$this->translate('Access'). '","' .$this->translate('Size (bytes)'). '","' .$this->translate('Duration (seconds)'). '","' .$this->translate('File type').'"');
	...
	$objSheet ->getCell('A'.$contador)->setValue('"'.$deposit_id. '","' .$deposit_title. '","' .$date. '","' .$bundle_id. '","' .$file_name. '","' .$access. '","' .$size. '","' .$duration. '","' .$file_type. '"' );
