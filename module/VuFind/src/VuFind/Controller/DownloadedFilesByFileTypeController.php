 ...
 * @link     http://vufind.org   Main Site
 */
class DownloadedFilesByFileTypeController extends AbstractBase
{
	...
	if (empty($dayTo)&& $dayTo<>"0") $dayTo = $current_day;
    	...
    	$downloadStat= $this->getTable('DownloadStat');
    	
	$downloadByFileType = $downloadStat->getByFileTypeDeposit($yearFrom, $monthFrom, $dayFrom, $yearTo, $monthTo, $dayTo,$fundingBody, $currentSuperseded,$deposit);
	...
	$results [] = $years;
        ...
        if (isset($_POST['btnExportarExcelDownloadedFilesByFileType'])){
                $this->exportarexcelDownloadedFilesByFileTypeDeposit($downloadByFileType,$yearFrom,$monthFrom,$dayFrom,$yearTo,$monthTo,$dayTo,$fundingBody, $currentSuperseded,$deposit, $source);
        }


        $view = $this->createViewModel(array('title' => $depositTitle, 'fundingbodies'=> $fundingBodies, downloadByFileType=>$downloadByFileType));
        $view->setTemplate('downloadedfilesbyfiletype/home');
    ...
    public function exportarexcelDownloadedFilesByFileTypeDeposit($results,$yearFrom,$monthFrom,$dayFrom,$yearTo,$monthTo,$dayTo,$fundingBody, $currentSuperseded,$deposit,$source){
