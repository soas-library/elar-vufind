...
class MyResearchController extends AbstractBase
{
    ...
    public function homeAction()
    {
        ...
        $this->getServiceLocator()->get('VuFind\MyresearchStats')
    ...
    public function resourceAction()
    {
		...
		$file = '/mnt/ELAR_Deposit_Resources/' . $id;		
		...
		if (is_file($file)) {
		    ...
		    $message = $this->translate("No file");
                }
    ...
    public function resourcesAction()
    {
        ...
        switch ($format) {
            ...
            case 'rar':
                $validFormat=1;
                $content = "application/x-rar-compressed";
                break;
            case 'htm':
                $validFormat=1;
                $content = "text/html";
        ...
        if ($user->major>1) {
            if ($validFormat){
                $filename = $id1 . '_'. $id2 . '_'. $id3 . '.'.$format;

                $file = '/usr/local/vufind/local/cache/resources/' . $filename;
                if (is_readable($file)) {
                ...
                $message =  $this->translate("Invalid format");
            }
        }
        else {
                $message =  $this->translate("User is not internal");
        }
