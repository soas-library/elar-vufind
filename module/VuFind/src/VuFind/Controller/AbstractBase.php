    ...
    implements AuthorizationServiceAwareInterface
{
    ...
    protected function createEmailViewModel($params = null, $defaultSubject = null)
    {
        ...
        $user = $this->getUser();
	$view->nodeid = $this->params()->fromQuery('nodeid');
	$view->userid = $this->params()->fromQuery('userid');
	$view->userfirst = $user->firstname;
	$view->userlast = $user->lastname;
	$view->filename = $this->params()->fromQuery('filename');
        //SCB Change
        if (!empty($user->displayed))
        $view->userfrom = $user->displayed;
        else
        $view->userfrom = $user->email;
        ...
        if ($this->getRequest()->isPost()) {
            ...
            $view->access = $this->params()->fromPost('optradio');
            
            $view->datefrom = $this->params()->fromPost('datefrom');
            $view->dateto = $this->params()->fromPost('dateto');
