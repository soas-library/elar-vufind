 ...
 * @link     http://vufind.org   Main Site
 */
class UploadedFilesController extends AbstractBase
{
    /**
	...
	$objSheet ->getCell('A'.$contador)->setValue($this->translate('Upload date'). ',' .$this->translate('Time'). ',"' .$this->translate('bundle ID'). '","' .$this->translate('Status'). '","' .$this->translate('Bundle title').'","' .$this->translate('File name'). '","' .$this->translate('Access'). '","' .$this->translate('Size (bytes)'). '","' .$this->translate('Duration (seconds)'). '","' .$this->translate('File type').'"');
	...
	foreach ($results as $valor) {
		...
		$objSheet ->getCell('A'.$contador)->setValue(date("j F Y", $timestamp). ',' .$stat[1]. ',"' .explode("-", $stat[2])[1]. '","' .$stat[11]. '","'  .end(explode('/',$stat[3])). '","' .end(explode('/',$stat[4])). '","' .$stat[5]. '","' .$stat[6]. '","' .$stat[7]. '","' .$stat[8] . '"');
