...
class Search extends Gateway
{
     ...
     * @return void
     */
    public function saveSearchWithUser(\VuFind\Search\Results\PluginManager $manager,
        $newSearch, $sessionId, $searchHistory = [],$user_id
    ) {
        // Duplicate elimination
        foreach ($searchHistory as $oldSearch) {
            // Deminify the old search (note that if we have a resource, we need
        ...
        $newSearch->updateSaveStatus($row);
        $row->search_object = serialize(new minSO($newSearch));
        if (!is_null($user_id)) $row->user_id = $user_id;
        //$row->user_id = is_null($user_id) ? $user_id : '0'
        //$row->user_id = $user_id;
        $row->save();
    }
}