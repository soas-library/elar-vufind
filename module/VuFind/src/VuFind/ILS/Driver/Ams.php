    ...
    public function getResource($id, $email, $setSpec, array $patron = null)
    {
        ...
        try {
            ...
            foreach ($itemSqlStmt->fetchAll() as $rowItem) {
                ...
                //echo $sql. "</br></br>";
		$deposit_id = $rowItem['vpath2'];
                try {
                    $itemSqlStmt = $this->db->prepare($sql);
                    $itemSqlStmt->execute();
                    foreach ($itemSqlStmt->fetchAll() as $rowItem2) {
                        $roles_bundle_id = $rowItem2['aclstring'];
                    }
		    // Deposit roles
		    $sql = "SELECT aclstring from accessgroups where md5 like (SELECT writerights from archiveobjects where nodeid=" . $deposit_id .")";
		    //echo $sql. "</br></br>";
		    $itemSqlStmt = $this->db->prepare($sql);
                    $itemSqlStmt->execute();
                    foreach ($itemSqlStmt->fetchAll() as $rowItem2) {
                        $roles_deposit_id = $rowItem2['aclstring'];
                    }
		...
		$autorised="0";
                $anyuser="0";
                $everybody="0";
		//Check if email is authorised in a specific deposit
	
		//echo $deposit_id. "</br></br>";
		//echo 'Roles deposit'. "</br></br>";
		//echo $roles_deposit_id. "</br></br>";	

	        $posEmailDeposit = strpos($roles_deposit_id, $email);
                //Check if email is authorised in a specific resource
                
		//echo 'Roles bundles'. "</br></br>";
		//echo $roles_bundle_id. "</br></br>";

		$posEmailBundle = strpos($roles_bundle_id, $email);
                //Check if the resource is free for everybody
                $posEverybody = strpos($roles_bundle_id, "everybody");
                $posAnyAuthenticatedUser = strpos($roles_bundle_id, "anyAuthenticatedUser");
                ...
                if ($posEmailDeposit !== false || $posEmailBundle !== false || $posEverybody !== false ) {
            ...
            foreach($resource as $item){
                $sql = "select imdimd_mediafile.nodeid as internal_node_id, type, format, accessavailability, replace(replace(replace(resourcelink,'file:/lat/corpora/zzz-test-zzz/',''),'file:/lat/corpora/ELAR/',''),'https://lat1.lis.soas.ac.uk/corpora/ELAR/','') as path, project_id, funding_body ".
                "from imdimd_mediafile, elar_imdimd_extra ".
                "where elar_imdimd_extra.nodeid=".$deposit_id. " " .
                "and (resourcelink = '". $item['url']."' ".
                "or resourcelink = '". str_replace('file:/lat/','https://lat1.lis.soas.ac.uk/',trim($item['url']))."') ".
                "UNION select imdimd_writtenresource.nodeid as internal_node_id, type, format, accessavailability, replace(replace(replace(resourcelink,'file:/lat/corpora/zzz-test-zzz/',''),'file:/lat/corpora/ELAR/',''),'https://lat1.lis.soas.ac.uk/corpora/ELAR/','') as path, project_id, funding_body ".
                "from imdimd_writtenresource, elar_imdimd_extra ".
                "where elar_imdimd_extra.nodeid=".$deposit_id. " " .
                "and (resourcelink = '". $item['url']."' ".
                "or resourcelink = '". str_replace('file:/lat/','https://lat1.lis.soas.ac.uk/',trim($item['url']))."') ";
                ...
                try {
                    ...
                    foreach ($itemSqlStmt->fetchAll() as $rowItem3) {
                        ...
                        $internal_node_id = $rowItem3['internal_node_id'];
                        ...
                        $sqlResource=" SELECT cs.nodeid, ao.url, CASE WHEN readrights = 'anyAuthenticatedUser' then 'U'	WHEN readrights = 'everybody' THEN 'O' ELSE CASE WHEN accesslevel = 2 THEN 'U' ELSE 'S' END END as accessavailability ".
                        "FROM corpusstructure cs, archiveobjects ao ".
                        "WHERE cs.nodeid  = '".$item['nodeid']."' AND cs.nodeid = ao.nodeid";
                        //echo "sql resource --" . $sqlResource."\n";

                        try {
                            $itemSqlStmt = $this->db->prepare($sqlResource);
                            $itemSqlStmt->execute();
                            foreach ($itemSqlStmt->fetchAll() as $rowResource) {
                                $accessavailability = $rowResource['accessavailability'];
                            }
                        }
                        catch (PDOException $e) {
                            throw new ILSException($e->getMessage());
			...
			try
			{
			    ...
			    "and replace(resourcelink,'file:/lat/corpora/ELAR/','') = '". $rowItem3['path'] ."'";
                            //echo $sqlSizeDuration. "</br></br>";

			    $itemSqlStmt = $this->db->prepare($sqlSizeDuration);
			    $itemSqlStmt->execute();
			    ...
			    throw new ILSException($e->getMessage());
                    	...
                    	$finalResource[] =[
                            ...
                            'everybody' => $item['everybody'],
                            'project_id' => $rowItem3['project_id'],
                            'funding_body' => $rowItem3['funding_body'],
                            'size' => $size,
                            'duration' => $duration,
...
/**
     * Get BundleID and one resource
     *
     * Retrieve the bundle id and the selected resource from nodeid.
     *
     * @param int  $node_id Node ID
     */
    public function getBundlesAndOneResource($node_id)
    {
        $this->db = new PDO(
            'pgsql:host=' . $this->config['CatalogStructure']['host'] .
            ';port=' . $this->config['CatalogStructure']['port'] .
        ...
        $sql = "select vpath from corpusstructure where nodeid = ". $node_id .";";
        ...
        $sql = "select name from corpusnodes where nodeid = ". $node_id .";";
     ...
     /**
     * Get BundleID and all child resources
     *
     * Retrieve the bundle id and all child resources from nodeid.
     *
     * @param int  $node_id Node ID
     */
    public function getBundlesAndResources($node_id)
    {
        $this->db = new PDO(
            'pgsql:host=' . $this->config['CatalogStructure']['host'] .
            ';port=' . $this->config['CatalogStructure']['port'] .
        ...
        $sql = "select nodeid from corpusstructure where vpath like '%". $vpath ."%';";
        
        //Bundle ID
        $bundleId = str_replace("#","",$bundleId);
        //$bundleId = str_replace("MPI","",$bundleId);
        $all[] = $bundleId;
        ...
        try {
            ...
            $itemSqlStmt->execute();
            foreach ($itemSqlStmt->fetchAll() as $rowItem) {
                $resource[] = $rowItem['nodeid'];
                
                $sql_names = "select name from corpusnodes where nodeid = ". $rowItem['nodeid'] .";";
                $itemSqlStmt_names = $this->db->prepare($sql_names);
                $itemSqlStmt_names->execute();
                foreach ($itemSqlStmt_names->fetchAll() as $rowItemName) {
                	$resourceName[] = $rowItemName['name'];
                }
     ...
     * @param int  $node_id Node ID
     */
    public function getCollectionAndBundles($node_id)
    {
        $this->db = new PDO(
            'pgsql:host=' . $this->config['CatalogStructure']['host'] .
            ';port=' . $this->config['CatalogStructure']['port'] .
        ...
        $sql = "select * from corpusstructure where nodeid = ". $bundleId .";";
        ...
        //$collectionId = str_replace("MPI","",$collectionId);
        $all[] = $collectionId;
