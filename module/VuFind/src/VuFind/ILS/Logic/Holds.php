...
class Holds
{
    ...
    /**
     * SCB New public method for getting resources
     *
     * @param string $id  A Bib ID
     * @param array  $ids A list of Source Records 
     *
     * @return array A sorted results set
     */
    public function getResources($id, $ids = null, $email, $setSpec)
    {
        $holdings = [];
        // Get Holdings Data
        if ($this->catalog) {
            ...
            if (isset($config['consortium']) && $config['consortium'] == true) {
                ...
                $result = $this->catalog->getResource($id, $email, $setSpec, $patron ? $patron : null);
            }
    ...
    /**
     * SCB New public method for getting depositor from node id
     *
     * @param string $id  A Bib ID
     * @param array  $ids A list of Source Records 
     *
     * @return array A sorted results set
     */
    public function getDepositor($nodeId)
    {
	$result = $this->catalog->getDepositorFrom($nodeId);
        return $result;
    }
    
    /**
     * SCB New public method for getting permission from node id
     *
     ...
     */
    public function getUserPermission($u, $node_id)
    {
	$result = $this->catalog->getStadisticsPermission($u, $node_id);
    	return $result;
    }

    /**
     * SCB New public method for getting bundle id and one resource
     *
     * @param int  $node_id Node ID
     *
     * @return array ids
     */
    public function getBundlesIdAndOneResource($node_id)
    {
	$result = $this->catalog->getBundlesAndOneResource($node_id);
    	return $result;
    }

    /**
     * SCB New public method for getting bundle id and all child resources
     *
     * @param int  $node_id Node ID
     *
     * @return array ids
     */
    public function getBundlesIdAndResources($node_id)
    {
	$result = $this->catalog->getBundlesAndResources($node_id);
    	return $result;
    }

    /**
     * SCB New public method for getting collection id and all child bundles
     *
     * @param int  $node_id Node ID
     *
     * @return array ids
     */
    public function getCollectionIdAndBundles($node_id)
    {
	$result = $this->catalog->getCollectionAndBundles($node_id);
    	return $result;
    }
