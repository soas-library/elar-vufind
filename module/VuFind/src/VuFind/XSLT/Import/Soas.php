...
class Soas 
{
    ...
    /**
     * Get the funding body of one element
     *
     * @param string $setSpec. Information of setSpec
     *
     * @return string      element id
     */
    public static function getFundingBody($setSpec)
    {
	...
	$secondPos = strpos($aux, ":");
        ...
        $sql = "SELECT  funding_body, project_id, nodeid " .
            "from elar_imdimd_extra ".
            "where nodeid = ". str_replace("MPI","",$parent). "";
        //echo $sql;
        try {
            $itemSqlStmt = $db->prepare($sql);
            $itemSqlStmt->execute();
            foreach ($itemSqlStmt->fetchAll() as $rowItem) {
                $funding_body =  $rowItem['funding_body'];
            }
        }
        ...
        $funding_body = ltrim($funding_body,"{");
        $funding_body = rtrim($funding_body,"}");
        $stringList = explode(",", $funding_body);
    ...
    /**
     * Get the project_id of one element
     *
     * @param string $setSpec. Information of setSpec
     *
     * @return string      element id
     */
    public static function getProjectId($setSpec)
    {
        $projectId = "";
    	$parent="";
    	$firstPos=strpos($setSpec,":");
	...
	$secondPos = strpos($aux, ":");
        ...
        try {
            ...
            $itemSqlStmt->execute();
            foreach ($itemSqlStmt->fetchAll() as $rowItem) {
                $funding_body =  $rowItem['project_id'];
            }
        }
        catch (PDOException $e) {
            throw new ILSException($e->getMessage());
            ...
            foreach ($itemSqlStmt->fetchAll() as $rowItem) {
                ...
                try {
                    ...
                    foreach ($itemSqlStmt->fetchAll() as $rowItem2) {
                        ...
                        "WHERE cs.nodeid  = '".$rowItem['nodeid']."' AND cs.nodeid = ao.nodeid";
                        //echo "sql resource --" . $sqlResource."\n";

                        try {
                            $itemSqlStmt = $db->prepare($sqlResource);
                            $itemSqlStmt->execute();
                            foreach ($itemSqlStmt->fetchAll() as $rowResource) {
                                $accessLevel  = $rowResource['accessavailability'];
                            }
                        }
                        catch (PDOException $e) {
                            throw new ILSException($e->getMessage());
