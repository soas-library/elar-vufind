...
class Importer implements ServiceLocatorAwareInterface
{
    ...
    public function save($xmlFile, $properties, $index = 'Solr',
        ...
        //SCB Allow multiple docs in <add></add> section
	$xmlNew = str_replace("<?xml version=\"1.0\" encoding=\"utf-8\"?>","<?xml version=\"1.0\" encoding=\"utf-8\"?><add>",$xml);
	$xmlNew = $xmlNew . '</add>';
	$xml = $xmlNew;
	// SCB End Allow multiple docs in <add></add> section
    ...
    protected function generateXML($xmlFile, $properties)
    {
        ...
        $xml = new DOMDocument;
        // SCB Avoid invalid names
        $xmlAux = file_get_contents($xmlFile);
        $final = preg_replace ('/[^\x{0009}\x{000a}\x{000d}\x{0020}-\x{D7FF}\x{E000}-\x{FFFD}]+/u', ' ', $xmlAux);
        file_put_contents($xmlFile, $final);
        // SCB Avoid invalid names
        if (!$xml->load($xmlFile)) {
...
libxml_get_errors();
            //$this->display_xml_errors ( libxml_get_errors() );
