<xsl:stylesheet version="1.0"
	...
	<xsl:apply-templates select="imdi:Session/imdi:Resources/imdi:WrittenResource"/>
    </xsl:template>
    ...
    <xsl:template match="imdi:Session/imdi:Resources/imdi:WrittenResource">
         <xsl:element name="add">
            <doc>
                <!-- Record ID and Hierarchy info -->            
                <!-- RECORDTYPE -->
		...
		<!-- Original - changing this as suspect it is best to generate individual child record for each resource which incorporates bundle info where required!
                ...
                -->
                <field name="id">
                	<xsl:value-of select="//imdi:METATRANSCRIPT/imdi:Session/imdi:MDGroup/imdi:Keys/imdi:Key[@Name='CorpusId']"/>-<xsl:value-of select="//imdi:METATRANSCRIPT/imdi:Session/imdi:Resources/imdi:WrittenResource/@ResourceId"/>
                </field>
		...
		<!-- NB imdi:FullName does not always contain a value sometimes no name provided in ELDP Profile sheets so used perso id but only in name field -->
                ...
                <xsl:for-each select="//imdi:METATRANSCRIPT/imdi:Session/imdi:Resources/imdi:WrittenResource/imdi:Keys/imdi:Key[@Name='Keyword']">
                ...
                <field name="resource_access_protocol">
                        <xsl:value-of select="//imdi:METATRANSCRIPT/imdi:Session/imdi:Resources/imdi:WrittenResource/imdi:Access/imdi:Availability"/>
                </field>
                <!-- Resource file path  -->
                <field name="resource_link">
                        <xsl:value-of select="//imdi:METATRANSCRIPT/imdi:Session/imdi:Resources/imdi:WrittenResource/imdi:ResourceLink"/>
                </field>
                <!-- Resource file type - will need work in source imdi - possible custom key field -->
                <field name="resource_type">
                        <xsl:value-of select="//imdi:METATRANSCRIPT/imdi:Session/imdi:Resources/imdi:WrittenResource/imdi:Type"/>
                </field>
           </doc>
	</xsl:element>        
    </xsl:template>
