<xsl:stylesheet version="1.0"
	...
	<xsl:apply-templates select="imdi:Session/imdi:MDGroup/imdi:Project"/>
	<xsl:apply-templates select="imdi:Session/imdi:Resources/imdi:MediaFile"/>
    </xsl:template>


    <!-- Process a deposit -->
    <xsl:template match="imdi:METATRANSCRIPT/imdi:Session/imdi:MDGroup/imdi:Project">
        <xsl:element name="doc">
                <!-- Record ID and Hierarchy info -->
                <!-- RECORDTYPE -->
                <field name="recordtype">Project</field>
                <!-- FORMAT -->
                ...
                <!-- ID -->
                <field name="id">
                        <xsl:value-of select="substring(//imdi:METATRANSCRIPT/imdi:Session/imdi:MDGroup/imdi:Project/imdi:Name,1,13)"/>
                </field>
                ...
                <field name="hierarchy_top_title">
                        <xsl:value-of select="//imdi:METATRANSCRIPT/imdi:Session/imdi:MDGroup/imdi:Project/imdi:Name"/>
                </field>
		...
		<!-- Project (Deposit) Name -->
                <!-- BUNDLE_CONTINENT -->
                <field name="bundle_continent">
                        <xsl:value-of select="//imdi:METATRANSCRIPT/imdi:Session/imdi:MDGroup/imdi:Location/imdi:Continent"/>
                </field>
                ...
                <field name="deposit_title">
                        <xsl:value-of select="//imdi:METATRANSCRIPT/imdi:Session/imdi:MDGroup/imdi:Project/imdi:Title"/>
                </field>
                <!-- Project description -->
		<xsl:for-each select="//imdi:METATRANSCRIPT/imdi:Session/imdi:MDGroup/imdi:Project/imdi:Description">
		...
		<!-- Project Cover image -->
                <field name="deposit_cover_image">
                        <xsl:value-of select="//imdi:METATRANSCRIPT/imdi:Session/imdi:MDGroup/imdi:Keys/imdi:Key[@Name='CoverImage']"/>
                </field>
		<!--Depositor-->
                <field name="depositor">
                        <xsl:value-of select="//imdi:METATRANSCRIPT/imdi:Session/imdi:MDGroup/imdi:Actors/imdi:Actor/imdi:Name"/>
                </field>
                <field name="depositor_image">
                        <xsl:value-of select="//imdi:METATRANSCRIPT/imdi:Session/imdi:MDGroup/imdi:Actors/imdi:Actor/imdi:Keys/imdi:Key/@filepath"/>
                </field>
	</xsl:element>
    </xsl:template>
    ...
    <xsl:template match="imdi:Session/imdi:Resources/imdi:MediaFile">
		...
		<!-- Original - changing this as suspect it is best to generate individual child record for each resource which incorporates bundle info where required!
                <field name="id">
                           <xsl:value-of select="ancestor::identifier"/>
                </field>
                -->
                <field name="id">
                	<xsl:value-of select="ancestor::imdi:METATRANSCRIPT/imdi:Session/imdi:MDGroup/imdi:Keys/imdi:Key[@Name='CorpusId']"/>-<xsl:value-of select="@ResourceId"/>
                </field>

                <!-- hierarchy_top_id used to enable simple linking to patent corpor record -->
                <field name="hierarchy_top_id">
                <xsl:value-of select="ancestor::imdi:METATRANSCRIPT/imdi:Session/imdi:MDGroup/imdi:Keys/imdi:Key[@Name='CorpusId']"/>
                </field>
                ...
                <!-- Bundle Location (string)-->
                <field name="bundle_location_string">
                     <xsl:value-of select="ancestor::imdi:METATRANSCRIPT/imdi:Session/imdi:MDGroup/imdi:Location/imdi:Address"/>
                </field>
                <!-- Bundle Description -->
                <field name="bundle_description">
                        <xsl:value-of select="ancestor::imdi:METATRANSCRIPT/imdi:Session/imdi:Description"/>
                </field>
                ...
                <field name="bundle_genre">
                       <xsl:value-of select="ancestor::imdi:METATRANSCRIPT/imdi:Session/imdi:MDGroup/imdi:Content/imdi:Genre"/>
                </field>
                <!-- Bundle Genre (string) -->
		...
		<!-- NB imdi:FullName does not always contain a value sometimes no name provided in ELDP Profile sheets so used perso id but only in name field -->
                ...
                <xsl:for-each select="imdi:Keys/imdi:Key[@Name='Keyword']">
                ...
                <field name="resource_access_protocol">
                        <xsl:value-of select="imdi:Access/imdi:Availability"/>
                </field>
                <!-- Resource file path  -->
                <field name="resource_link">
                        <xsl:value-of select="imdi:ResourceLink"/>
                </field>
                <!-- Resource file type - will need work in source imdi - possible custom key field -->
                <field name="resource_type">
                        <xsl:value-of select="imdi:Type"/>
                </field>
                <!-- Resource file type (string) -->
